...
class _UserManagementPageState extends State<UserManagementPage> {
  ...
  Future<void> _ajouterUtilisateur(StateSetter dialogSetState) async {
    ...
    try {
      ...
      _showSuccessMessage('Utilisateur "${_prenomController.text} ${_nomController.text}" ajouté avec succès');
    } catch (e) {
  ...
  Future<void> _modifierUtilisateur(String id, StateSetter dialogSetState) async {
    if (!_formKey.currentState!.validate()) return;
    ...
    try {
      ...
      dialogSetState(() => _isLoading = false);
      Navigator.of(context).pop();
      _showSuccessMessage('Utilisateur "${_prenomController.text} ${_nomController.text}" modifié avec succès');
    } catch (e) {
  ...
  Future<void> _supprimerUtilisateur(String id, String nom, String prenom) async {
    final result = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Confirmer la suppression'),
        content: Text('Êtes-vous sûr de vouloir supprimer l\'utilisateur "$prenom $nom" ?\n\nCette action est irréversible.'),
        actions: [
          TextButton(
  ...
  Future<void> _toggleUserStatus(String id, bool currentStatus, String nom, String prenom) async {
    try {
      await _firestore.collection('utilisateurs').doc(id).update({
        'actif': !currentStatus,
        'dateModification': FieldValue.serverTimestamp(),
      });
      _showSuccessMessage(currentStatus
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          userId == null ? 'Ajouter un utilisateur' : 'Modifier l\'utilisateur',
                      ...
                      ElevatedButton(
                        onPressed: _isLoading ? null : () {
                          if (userId == null) {
                            ...
                            _modifierUtilisateur(userId, setState);
                          }
                        },
                        ...
                        child: _isLoading
                          ...
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(userId == null ? Icons.add : Icons.save, size: 18),
                            const SizedBox(width: 8),
                            Text(userId == null ? 'Ajouter' : 'Modifier'),
                          ],
                        ),
                      ),
                    ],
        ...
        children: [
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: _firestore.collection('utilisateurs').orderBy('dateCreation', descending: true).snapshots(),
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        leading: CircleAvatar(
                          radius: 25,
                          backgroundColor: data['actif'] == true ? Colors.green.shade100 : Colors.red.shade100,
                          child: data['actif'] == true
                              ? Icon(Icons.person, color: Colors.green.shade700, size: 28)
                              : Icon(Icons.person_off, color: Colors.red.shade700, size: 28),
                        ),
                        title: Text(
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            ...
                            if (data['telephone'] != null && data['telephone'].toString().isNotEmpty) ...[
                            ...
                            Wrap(
                              ...
                              children: [
                                ...
                                Container(
                                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                                  decoration: BoxDecoration(
                                    color: (data['actif'] == true ? Colors.green : Colors.red).withOpacity(0.1),
                                    borderRadius: BorderRadius.circular(12),
                                  ),
                                  child: Row(
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      Icon(
                                        data['actif'] == true ? Icons.check_circle : Icons.block,
                                        size: 14,
                                        color: data['actif'] == true ? Colors.green : Colors.red,
                                      ),
                                      const SizedBox(width: 4),
                                      Text(
                                        data['actif'] == true ? 'Actif' : 'Inactif',
                                        style: TextStyle(
                                          color: data['actif'] == true ? Colors.green : Colors.red,
                                          fontWeight: FontWeight.w500,
                                          fontSize: 12,
                          ...
                          onSelected: (value) {
                            ...
                            switch (value) {
                              case 'modifier':
                                ...
                                break;
                              case 'toggle_status':
                                _toggleUserStatus(userId, data['actif'] ?? true, nom, prenom);
                                break;
                              case 'supprimer':
                          ...
                          itemBuilder: (context) => [
                            ...
                            PopupMenuItem(
                              ...
                              child: Row(
                                children: [
                                  Icon(
                                    data['actif'] == true ? Icons.block : Icons.check_circle,
                                    color: data['actif'] == true ? Colors.orange : Colors.green,
                                  ),
                                  const SizedBox(width: 8),
                                  Text(data['actif'] == true ? 'Désactiver' : 'Activer'),
                                ],
                              ),
                            ),
                            const PopupMenuItem(
